...
`identity_*` variables in `group_vars/all.yaml`, which the `authentik` role
renders into the LDAP blueprint from the same definitions (ADR-0050).

The LDAP certificate is the `homelab-ldap` certificate generated by the
`authentik` role for `identity_ldap_host`. Authentik's built-in certificate
...
| `sssd_ldap_bind_dn` | `cn={{ identity_ldap_bind_user }},ou=users,…` | Lookup account |
