...
| Shared platform ServiceMonitors | `charts/prometheus/templates/{argocd,cilium-servicemonitors,envoy-gateway,external-dns}.yaml` | `ServiceMonitor` and `PodMonitor` resources for bootstrap/platform apps | Bootstrap/cross-cutting scrape resources | Acceptable centralization. These have CRD ordering or cross-cutting consistency reasons; the templates carry bidirectional counterpart comments pairing them with `k8s/dev-monitoring`. |
...
2. Keep prd/dev duplicated platform scrape templates intentionally paired; when
   changing ArgoCD, Cilium, Envoy Gateway, or external-dns scraping, update
   `k8s/dev-monitoring` at the same time (the template comments point at the
   counterpart files).
